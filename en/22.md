...
We've already discussed three different methods for fixing versions. Strictly speaking, `git revert` and `git cherry-pick` don't really "fix version history records" - they apply previously committed versions, whether "re-applying" or "reverse-applying" them. This article will explain Git's Rebase mechanism, which is truly used to modify commit records. This feature is important and powerful.

## Understanding git rebase

Rebase is one of Git's most powerful but potentially dangerous features. It allows you to rewrite commit history, making it cleaner and more linear. 
