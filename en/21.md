...
In the version control process, there is another common situation: you've been developing in a branch for a while, but later decide you don't want the entire branch anymore. However, there are a few commits within it that you still want to keep. In this case, you can't delete the branch, but merging the entire branch back isn't appropriate either. What should you do? This article will explain how to use the `git cherry-pick` command to "manually select" the changes you want to apply.
